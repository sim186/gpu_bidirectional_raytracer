    ...
    unsigned int seed_id, unsigned int param, LightPath* dev_lp, int vlp_index)
{
	...
	if (x <= width)
	{                          // se TID del thread del device è minore dell'asse x del piano immagine, in sostanza sono finiti i pixel da assegnare
		...
		if (counter[i] < 30000)
		{ // A che serve? Sarebbe il numero di percorsi per tutta l'immagine?

			Ray currentRay;
			rassign(currentRay, ray);
			Vec rad;
